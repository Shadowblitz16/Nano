...
start:
	...
	; come BIOSes might start us at 07C0:0000 instead of 0000:7C00, make sure we are in the
	; expected location
	push es;
	push word .after
	retf
...
.after:
	...
	; compute size of root directory = (32 * numbers_of_entries) / bytes_per_sector
...
.root_dir_after:
	...
	mov dl, [ebr_drive_number]		; dl = drive number (we saved it previusly)
...
.found_kernel:
	...
	; di should have the adress to the entry
	mov ax, [di +26]
...
.load_kernel_loop:
	; Read next cluster
	mov ax, [kernel_cluster]		; first cluster = (kernel_cluster - 2) * sectors_per_cluster + start_sector
